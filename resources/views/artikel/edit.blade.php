...
@section('content')
<h1 style="font-size:28px;font-weight: bold;color:navy">Ubah Artikel</h1>
		...
		<div class="row">
			<div class="col-xl-8 col-lg-7">                         
				<div class="card-body">
                  ...
                  <option value="{{ $item->id }}" {{ old('category_id', $artikel->category_id) == $item->id ? 'selected' : '' }}>{{ ucfirst($item->nama) }}</option>
                  @endforeach	
								</select>
							</div>
							<div class="form-group">
								<textarea cols="80" id="editor1" name="konten" rows="10">{{$artikel->konten}}</textarea>
							</div>
						</div>
					</div>                                         
				</div>
					...
					<div class="card-body">
						<div class="">
							...
							<a href="#" class="thumbnail">
								<img id="preview-image-before-upload" src="{{asset($artikel->gambar)}}" width="100%">
							</a>
						</div>                                  
					</div>                           
				</div>
